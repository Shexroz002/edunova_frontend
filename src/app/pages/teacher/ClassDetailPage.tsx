...
function getClassDetails(cls: ClassItem) {
  ...
  const quizzes = [
    { name: `${cls.subject} — 1-bo'lim testi`,    date: '14 mart', avgScore: Math.min(98, 72 + seed % 20), submissions: Math.min(cls.students, 22 + seed % 8) },
    { name: `${cls.subject} — Amaliyot testi`,    date: '10 mart', avgScore: Math.min(98, 65 + seed % 25), submissions: Math.min(cls.students, 18 + seed % 6) },
    { name: `${cls.subject} — Oraliq nazorat`,    date: '5 mart',  avgScore: Math.min(98, 78 + seed % 18), submissions: Math.min(cls.students, 25 + seed % 5) },
    { name: `${cls.subject} — Mustaqil topshiriq`,date: '1 mart',  avgScore: Math.min(98, 60 + seed % 30), submissions: Math.min(cls.students, 15 + seed % 7) },
...
export function ClassDetailPage() {
  ...
  const cls = ALL_CLASSES.find((c) => c.id === parseInt(id || '1', 10)) ?? ALL_CLASSES[0];
  const { students, quizzes } = getClassDetails(cls);
                  ...
                  >
                    ...
                    <div className="mt-2 w-full h-1 rounded-full overflow-hidden" style={{ background: t.border }}>
                      <div className="h-1 rounded-full transition-all duration-500"
                        style={{ width: `${(q.submissions / cls.students) * 100}%`, background: t.accent, opacity: 0.7 }} />
                    </div>
                  </div>
                );
              })}
